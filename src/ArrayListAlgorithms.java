...
public class ArrayListAlgorithms
{
    ...
    public static int indexOfMinimum(ArrayList<Integer> intList)
    {
        ...
        for(int i = 0; i < intList.size(); i++)
        {
            if(intList.get(i) < currentmin)
            {
                currentmin = intList.get(i);
                currentminindex = i;
            }
        }
    ...
    public static ArrayList<String> parseWordsAndReverse(String sentence)
    {
        ...
        String reversed = new String(sentence);
        for(int i = sentence.length(); i > 0; i--)
        {
            ret.add("");
            for(int j = i; j > 0 && sentence.charAt(i - 1) != ' '; j--)
            {
                String whut = ret.get(ret.size() - 1);
                whut += sentence.substring(j - 1, j);
                ret.set(ret.size() - 1, whut);
                i--;
            }
        }
        for(int i = 0; i < ret.size(); i++)
        {
            ...
            {
                String reverse = ret.get(i);
                unreverse += reverse.charAt(j);
            }
            ret.set(i, unreverse);
        }
        return ret;
    ...
    {
        ArrayList<String> bword = new ArrayList<>();
        for(int i = 0; i < wordList.size(); i++)
        {
            if(wordList.get(i).charAt(0) == 'b')
            {
                bword.add(wordList.get(i));
                wordList.remove(i);
                i--;
            }
        }
    ...
    public static ArrayList<Integer> modes(int[] numList)
    {
        ...
        for(int i = 0; i < numList.length; i++)
        {
            ...
            if(count > currentMost)
            {
                currentMost = count;
                modes.clear();
                modes.add(numList[i]);
            }
            else if(count == currentMost && count != 1)
            {
                modes.add(numList[i]);
            }
        }
